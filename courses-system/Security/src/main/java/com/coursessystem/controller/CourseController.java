...
public class CourseController {
    ...
    public ResponseEntity<Course> get(@RequestHeader("Authorization") String authorization, @PathVariable Long id) {
    	try {
    ...
    public ResponseEntity<List<Course>> getAll(@RequestHeader("Authorization") String authorization) {
        return new ResponseEntity<>(courseService.getAll(), HttpStatus.OK);
    }

    @PostMapping
	@ApiOperation(value = "createCourse", notes = "Crear curso", response = ResponseGeneral.class)
    public ResponseEntity<ResponseGeneral> register(@RequestHeader("Authorization") String authorization, @RequestBody @Valid PostCourse course, BindingResult result) {
		if (result.hasErrors())
    ...
    public ResponseEntity<ResponseGeneral> register(@RequestHeader("Authorization") String authorization, @RequestBody @Valid SubscribeRequest request, BindingResult result) {
		if (result.hasErrors())
            return new ResponseEntity<>(new ResponseGeneral(HttpStatus.BAD_REQUEST, result.getFieldError().getField() + " " + result.getFieldError().getDefaultMessage()), HttpStatus.BAD_REQUEST);
		try {
    ...
    public ResponseEntity<ResponseGeneral> update(@RequestHeader("Authorization") String authorization, @RequestBody @Valid Course user, BindingResult result) {
		if (result.hasErrors())
            return new ResponseEntity<>(new ResponseGeneral(HttpStatus.BAD_REQUEST, result.getFieldError().getField() + " " + result.getFieldError().getDefaultMessage()), HttpStatus.BAD_REQUEST);
		try {
    ...
    public ResponseEntity<ResponseGeneral> delete(@RequestHeader("Authorization") String authorization, @PathVariable Long id) {
